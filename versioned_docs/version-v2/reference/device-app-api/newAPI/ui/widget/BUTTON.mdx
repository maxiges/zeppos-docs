...
| w              | The width of widgets;If set to `-1`, the size of normal_src is preferred, otherwise the default is `100`. | YES      | `number`    |
| h              | The height of widgets;If set to `-1`, the size of normal_src is preferred, otherwise the default is `40`. | YES      | `number`    |
...
| longpress_func | Callbacks for button long press(700ms).                                                                   | NO       | `ClickFunc` |
