---
title: BodyTemperature
sidebar_label: BodyTemperature
---

> Start from API_LEVEL `3.0` . Please refer to [API_LEVEL](../../../../guides/framework/device/compatibility.md).

Body surface temperature sensor.

:::info
permission code: `data:user.hd.body_temp`
:::

## Methods

### getCurrent

Get the latest measurement of body surface temperature

```ts
getCurrent(): Result
```

#### Result

| Property     | Type                | Description                                                                                             | API_LEVEL |
| ------------ | ------------------- | ------------------------------------------------------------------------------------------------------- | --------- |
| value        | <code>number</code> | Value of the last measured value. The value is given to 2 decimal places. for example 36.6 == 3660      | 3.0       |
| time         | <code>number</code> | The time that has passed after reading the value from the device's sensor. (Given in seconds)           | 3.0       |

### getToday

Get the body surface temperature measurement values for 24 hours a day. The array length is 24 \* 60 / 5 = 288, with an average measurement value every five minutes. The unit is Celsius, such as `35.2`. Data without measurement values is `-1000`

```ts
getToday(): Array<number>
```

## Example

```js
import { BodyTemperature } from '@zos/sensor'

const bodyTemperature = new BodyTemperature()

const result = bodyTemperature.getCurrent()
console.log(`Current temp: ${result.value/100}`)
```
